...
その後ユーザーの明示依頼で1台目へ同じ結合UF2を書き込んだ。
RP2ブートローダー離脱と`USB\VID_CAFE&PID_BAF2\CPB8E57K0313`の列挙は確認済み。
1台目の保存Mappingと実機入力は未確認であり、2台目の10回試験結果をそのまま
1台目の合格結果として扱わない。`Flash B Side`は押さない。
