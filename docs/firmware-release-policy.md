...
1台目への自動展開として扱わない。
2026-09-24、ユーザーの明示依頼で1台目をRP2ブートモードにし、
`G:\INFO_UF2.TXT`、候補UF2のSHA-256、A/B静的gateを再確認して同じ結合UF2を
書き込んだ。Gドライブが消え、Windowsで1台目の
`USB\VID_CAFE&PID_BAF2\CPB8E57K0313`が通常列挙された。
これは書き込み・USB起動の証拠であり、1台目のMapping内容やキー・マウスの
実機動作成功を意味しない。2台目の試験結果を1台目に自動転用しない。
本書冒頭の旧最安定運用版は復元用としてまだ置き換えない。
